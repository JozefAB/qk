...
class plgGurupaymentPaypaypal extends JPlugin{
	...
	function onSendPayment(&$post){
		...
		if(!$param['business'] || !$param['currency_code']){
			JError::raiseWarning(403, JText::_("Error Payment Processing: some of the params passed are empty"));
			return;
		}
	...
	function onCheckParams($params){
		...
		JError::raiseNotice(100, $title.': '.JText::_('PayPal Account (E-mail) is not set'));
		if($params->get('paypaypal_requre') && !$params->get('paypaypal_ipn'))
		JError::raiseNotice(100, $title.': '.JText::_('Use IPN (Requires for subscriptions) is not selected'));
	}
}
