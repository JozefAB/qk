...
class guruControllerguruMedia extends guruController {
		...
		$return .= '<div class="hasTip hasTooltip" data-toggle="tooltip" title="" data-placement="top" data-original-title="'.trim($description).'">
						<div class="row-fluid">
							...
							<div class="cVideo-Content span8">
								<b>'.trim($title).'</b>
								<div><a href="#" onclick="javascript:changeVideo(); return false;" class="btn btn-small creator-change-video">Change Video</a></div>
	...
	function ajaxAddMassVideo(){
		...
		if($host == "1"){ // YouTube
			$url = "https://gdata.youtube.com/feeds/api/playlists/".$playlist_id."?v=2&alt=json&max-results=".intval($per_page)."&start-index=".intval($start_with);
			...
			if($nVideo > 0){
				for($i=0; $i<$nVideo; $i++){
    				...
    				$video[$i]['media$group']['media$description']['$t'] = str_ireplace($breaks, "\r\n", $video[$i]['media$group']['media$description']['$t']); 
					...
					$table .= '	<tr>
									...
									<td>
										<img src="'.$video[$i]['media$group']['media$thumbnail']["0"]['url'].'" />
										<input type="hidden" name="image[]" value="'.$video[$i]['media$group']['media$thumbnail']["0"]['url'].'" />
									</td>
									<td>
										<textarea name="title[]">'.$video[$i]['title']['$t'].'</textarea>
										<input type="hidden" name="url[]" value="'.$video[$i]['link']["0"]['href'].'" />
									</td>
									<td>
										<textarea name="description[]">'.$video[$i]['media$group']['media$description']['$t'].'</textarea>
									</td>
								</tr>';
				}
			}
