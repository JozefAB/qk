...
function jwAllVideos( &$row, $parawidth=300, $paraheight=20, $parvwidth=400, $parvheight=300) {
	...
	//$mosConfig_live_site = explode('administrator/', $_SERVER["HTTP_REFERER"]);
...
function create_media_using_plugin($main_media, $configs, $aheight, $awidth, $vheight, $vwidth){
	...
	//echo "<pre>Main media: ";var_dump($main_media);
	//echo "<br /> Configs:";var_dump($configs);
	//echo "<br /> AHeight:";var_dump($aheight);
	//echo "<br /> AWidth:";var_dump($awidth);
	//echo "<br /> Vheight:";var_dump($vheigth);
	//echo "<br /> vwidth:";var_dump($vwidth);
	...
	if($main_media->type=='docs')
		{
			if($main_media->source=='url')
				$media = '<a target="_blank" href="'.$main_media->url.'">'.$main_media->name.'</a>';	
			if($main_media->source=='local')
				$media = '<a target="_blank" href="'.$the_base_link.'/'.$configs->docsin.'/'.$main_media->local.'">'.$main_media->name.'</a>';
		}
	*/
	if($main_media->type=='image')
		{

				$img_size = @getimagesize(JPATH_SITE.DS.$configs->imagesin.'/'.$main_media->local);
				...
				$media .= $the_base_link.DS.$configs->imagesin.'/'.$main_media->local.'" />';	
			}
		}					
