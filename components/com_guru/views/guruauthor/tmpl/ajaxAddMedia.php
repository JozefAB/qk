...
<embed width="'.$vwidth.'" height="'.$vheight.'" type="video/x-msvideo" src="'.$root.$configs->videoin."/".$the_media->local.'" name="plugin">
	...
	elseif($the_media->type=='video' && $extension=="mp4"){
		...
		$root=explode("components",$server);
		$root="http://".$root[0];
		$media = '<object width="670" height="400" data="'.$root.$configs->videoin."/".$the_media->local.'"></object>';
	}
...
if($the_media->type == 'image')	{
	...
	$url="http://".substr($url,0,strpos($url,"component"));
...
function parse_quiz ($id,$type){
	...
	if($the_media->type=='docs'){
		$the_base_link = explode('index.php', $_SERVER['HTTP_REFERER']);
	...
	if($the_media->type=='quiz'){		
		...
		for($i=0;$i<count($quiz_questions);$i++){
			...
			$media = $media.'<br /><div align="left" style="padding-left:30px;"><input type="submit" class ="btn btn-warning" value="Submit" onclick="get_quiz_result()" /></div>';	
