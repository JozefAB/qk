...
<!-- teacher list
------------------------------------------------------------>
<div id="teacherList" class="clearfix com-cont-wrap g_sect">

    <!-- title
    ------------------------------------------------------------>
    <h2 class="guru_authors_title page_title">
    ...
    if($layout == "1"){
            ...
            if($layout == "1"){//mini profile
                ...
                if($wrap == "1"){//no wrap
                    if($img_align == "0"){// left
						...
						if($read_more == "0"){
                            $rmore = '<a style="text-align:'.$read_align.'" class="btn btn-primary" href="'.JRoute::_('index.php?option=com_guru&view=guruauthor&layout=view&cid='.$author->id."-".JFilterOutput::stringURLSafe($author->name)."&Itemid=".$item_id).'">'.JText::_("GURU_READ_MORE").'</a>';
                        }
                ...
                elseif($wrap == "0"){//wrap
                    ...
                    elseif($img_align == "1"){// right
						...
						if($read_more == "0"){
                            $rmore = '<a class="btn btn-primary" href="'.JRoute::_('index.php?option=com_guru&view=guruauthor&layout=view&cid='.$author->id."-".JFilterOutput::stringURLSafe($author->name)."&Itemid=".$item_id).'">'.JText::_("GURU_READ_MORE").'</a>';
                        }
                        $return .= 			'<div class="'.$style_authors->authors_description.'" style="text-align:'.$bio_align.';"><p>'.$bio.'</p>'.$rmore.'</div>';
                        $return .= 		"</div>";
                        $return .= "</div>";
        ...
        while(isset($authors_array[$i])){
            $row = "";
            $row .= '<div class="teacher_row_guru g_row clearfix " >';
            ...
            for($j=0; $j<$cols; $j++){
                if(isset($authors_array[$i])){
                    $row .= '<div class="teacher_cell_guru g_cell span'.$span.'">'.$authors_array[$i++]."</div>";
                }
            }
            $row .= '</div>';
            echo $row;
...
<script>
	window.onload = equalHeight('teacher_cell_guru');
</script>