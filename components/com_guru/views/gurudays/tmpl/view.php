...
if($img_width>0 && $img_height>0)
{
	...
	$cfg_image = '<img style="margin:5px;" border="0" alt="" src="'.JURI::base().'images'.DS.'stories'.DS.$image_full_thumb.'" />';
}
else
	$cfg_image = '';
...
foreach ($the_task_list as $one_task_obj) 
	...
	if($one_task_obj)
		{ // begin if
			...
			//$status_color = "#DDDDDD";
	 		// we find the TASK STATUS for the day and we change the COLOR accordingly
	 		//$task_status = guruModelguruDays::find_task_status($my->id,$_GET['cid'], $day->ordering, $task->id);
...
{
	// guest - begin
	$task_list_for_day = guruModelguruDays::find_tasks_for_aday(intval($_GET['cid']));
