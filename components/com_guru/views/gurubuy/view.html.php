...
class guruViewguruBuy extends JViewLegacy {
	...
	function getPlugins(){
		...
		if(!empty($plugins)){
			$return = JText::_("GURU_BUY_PAYMENT_METH").': ' . JHTML::_('select.genericlist',  $plugins, 'processor', 'class="inputbox" size="1" ', 'value', 'text', $processor);
		} 
		else{
	...
	function refreshCoursesFromCart($all_product){
		...
		if(isset($all_product) && isset($user_courses)){
			foreach($all_product as $key=>$value){
				if(isset($user_courses[$value["course_id"]])){
		...
		$_SESSION["courses_from_cart"] = $all_product;
		return $all_product;
	}
	...
	function setPromoTest($total, $counter){
		...
		if($promo_details->typediscount == '0') {//use absolute values					
			...
			if($difference < 0){
				...
				if($currencypos == 0){		
					$_SESSION["discount_value"] =  $character." ".(float)($discount*$counter);
				}
				else{
					$_SESSION["discount_value"] =  (float)($discount*$counter)." ".$character;
				}
