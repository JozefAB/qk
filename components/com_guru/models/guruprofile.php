...
class guruModelguruProfile extends JModelLegacy {
	...
	function store(){		
		...
		if(!$this->existCustomer($id)){
			//insert
			$sql = "insert into #__guru_customer(`id`, `company`, `firstname`, `lastname`) values (".intval($id).", '".addslashes(trim($company))."', '".addslashes(trim($first_name))."', '".addslashes(trim($last_name))."')";
		}
		else{
			//update
			$sql = "update #__guru_customer set company='".addslashes(trim($company))."', firstname='".addslashes(trim($first_name))."', lastname='".addslashes(trim($last_name))."' where id=".intval($id);
		}		
		$db->setQuery($sql);
		if($db->query()){
			return true;
