...
class guruModelguruauthor extends JModelList {
	...
	function getStudents(){
		...
		$id_quiz = JRequest::getVar("qid", "");
		$and = "";
		
		$sql = "select `id` from #__guru_program where `author`=".intval($user->id);
		$db->setQuery($sql);
		$db->query();
		$courses_ids = $db->loadColumn();
		
		if($action == "passed"){
			$sql = "SELECT q1.`user_id`, q1.`quiz_id`, q1.`score_quiz`
					FROM `#__guru_quiz_taken` q1
					WHERE q1.`quiz_id`=".intval($id_quiz)." and q1.`time_quiz_taken_per_user` = (select min(`time_quiz_taken_per_user`) from `#__guru_quiz_taken` q2 where q2.`user_id`=q1.`user_id`)
					GROUP BY q1.`user_id`, q1.`quiz_id`";
			$db->setQuery($sql);
			$db->query();
			...
			if(isset($result) && count($result) > 0){
				...
				foreach($result as $key=>$value){
					...
					$percent = ($score_array["0"] * 100) / $score_array["1"];
				...
				if(intval($cid)>0 && trim($action) != ""){
					...
					elseif($action == "pass"){
						...
						if(isset($students) && count($students) > 0){
							$sql = "select `user_id`, `score_quiz` from #__guru_quiz_taken where `quiz_id`=".intval($final_id);
							$db->setQuery($sql);
							$db->query();
	...
	function getAuthorMyCourses(){
		...
		$sql = "SELECT * FROM #__guru_program WHERE author=".intval($user->id).$and;
	...
	function getAuthorMyMedia(){
		...
		$search_media= JRequest::getVar("search_media", "");
		$status		= JRequest::getVar("media_publ_status", "YN");
		$type		= JRequest::getVar('media_type', "");
		$media_category		= JRequest::getVar('media_category', "");
	...
	function getAuthorMyQuizzes(){
		...
		$course = JRequest::getVar("selectcoursesd", "");
		$type		= JRequest::getVar("quiz_select_type", "");
		$search		= JRequest::getVar('search_quiz', "");
		...
		$sql = "SELECT * FROM #__guru_quiz WHERE `author`=".intval($user->id).$condition;
	...
	public function getData(){
		...
		if(empty($this->_data)){
			...
			$limit		= JRequest::getVar('limit', 12, '', 'int');
	...
	function getAuthor(){
		...
		$cid = JRequest::getVar('cid', 0);
		if($item != 0){
			$sql = "SELECT params FROM #__menu WHERE id=".$item;
			$db->setQuery($sql);
			$db->query();
			$params = $db->loadColumn();
			$params = json_decode(@$params["0"]);
			if(isset($params) && isset($params->cid)){
				$cid = $params->cid;
			}
		}		
		...
		if(!empty($result)){
			$query = "SELECT id, alias, name, level, startpublish FROM #__guru_program WHERE author=".intval($result->userid)." AND published=1";
			$db->setQuery($query);
			$db->query();
	...
	function getAuthorDetails(){
		...
		$type = JRequest::getVar("author_type", "1", "", "int");
		$result = new StdClass();
	...
	function delete(){
		...
		foreach($cids as $cid){
			$sql = "DELETE FROM #__guru_program WHERE author=".intval($user->id)." and id=".intval($cid);
			$db->setQuery($sql);
			$db->query();
		}
		if (!$db->query() ){
   ...
   function getProgramRenewals($id = 0)
   {
	...
	function getFilters(){
		...
		$sql = "SELECT `lp`.`id` as pid,`name` as name 
				...
				on lp.id=ld.pid 
				where lp.`author` = ".intval($user_id)."
	...
	function getlistDaysJumps() {
		$pid=JRequest::getVar("progrid","0","","int");
		if ($pid==0) {
			$sql = "SELECT * FROM #__guru_days WHERE 
					pid = ( SELECT id FROM #__guru_program WHERE id in 
					( SELECT pid FROM #__guru_days GROUP BY pid ) 
	...
	function store_new_module(){		
		...
		$media_id = JRequest::getVar("db_media_1", "0");
...
<embed width="'.$vwidth.'" height="'.$vheight.'" type="video/x-msvideo" src="'.JURI::root().$configs->videoin."/".$the_media->local.'" name="plugin">
	...
	function getfile(){
		...
		$folder	= JRequest::getVar("directory","","post","string");
		$txt = JRequest::getVar("txt","0","get","int");
		$fileType = JRequest::getVar("type","","get","string");
		$db = JFactory::getDBO();
		if ($folder!='root') 
			$getin = DS.$folder; 
		else $getin='';
		...
		$selected_item_New = JRequest::getVar('selected_item_New', '');
		...
		if($this->_package->type == ""){
			if(isset($_SESSION["type"])){
				$this->_package->type = $_SESSION["type"];
			}
		...
		//start video
		$directory 	= JPATH_SITE.DS.$config->videoin;
		chmod($directory, 0755);	
		$directoryt = JPATH_SITE.DS.$config->videoin.$getin;		
		...
		foreach ($allfolders as $fille) {
			...
			$extension = $extension_array[count($extension_array)-1];
			$extension = strtolower($extension);
			if($extension =='doc' || $extension =='docx' || $extension =='txt' || $extension =='pdf' || $extension =='csv' || $extension =='htm' || $extension =='html' || $extension =='xhtml' || $extension =='xml' || $extension =='sxw' || $extension =='rtf' || $extension =='odt' || $extension =='css' || $extension =='odp' || $extension =='pps' || $extension =='ppt' || $extension =='sxi'){					
				if(in_array(trim($fille), $all_my_files)){
	...
	function storeMedia(){
		...
		$data = JRequest::get('post');	
		$data['code_v']=JRequest::getVar('code_v','','post','string', JREQUEST_ALLOWRAW);	
	...
	function getMedia(){
		...
		if($id==0){
			...
			$column_order = "ORDER BY reorder";
			if($sortquestion != ""){
				$column_order = "ORDER BY `text` ".$sortquestion;
			}
			
			$db->setQuery("SELECT count(*) FROM `#__guru_questions` WHERE qid='".$id."' ".$column_order);
			$db->query();
			$total = $db->loadColumn();
			$total = @$total["0"];
			$media->total = $total;
			...
			$db->setQuery("SELECT * FROM `#__guru_questions` WHERE qid='".$id."' ".$column_order." ".$sql_limit);
			$media->mmediam = $db->loadObjectList();
			
			$db->setQuery("SELECT id FROM `#__guru_questions` WHERE qid = '".$id."' ORDER BY reorder DESC LIMIT 1");
			$media->max_reo = $db->loadResult();
			$db->setQuery("SELECT id FROM `#__guru_questions` WHERE qid = '".$id."' ORDER BY reorder ASC LIMIT 1");
	...
	function updatequestion ($qtext,$qid,$a1,$a2,$a3,$a4,$a5,$a6,$a7,$a8,$a9,$a10,$answers) {
		$db = JFactory::getDBO();
		$sql = "UPDATE `#__guru_questions` SET 
	...
	function getTotalStudentsByQuizzId(){
		$db = JFactory::getDBO();
		$id = JRequest::getVar("id", "0");
		$sql = "select distinct(q.`user_id`) from #__guru_quiz_taken q, #__users u where `quiz_id`=".intval($id)." and q.`user_id`=u.`id`";
		$db->setQuery($sql);
		$db->query();
		$result = $db->loadColumn();
		return count($result);
	...
	function getAvgScoreByQuizzId(){
		...
		$sql = "SELECT q1.`user_id`, q1.`quiz_id`, q1.`score_quiz`
				FROM `#__guru_quiz_taken` q1
				WHERE q1.`quiz_id`=".intval($id)." and q1.`time_quiz_taken_per_user` = (select max(`time_quiz_taken_per_user`) from `#__guru_quiz_taken` q2 where q2.`user_id`=q1.`user_id`)
				GROUP BY q1.`user_id`, q1.`quiz_id`";
		$db->setQuery($sql);
		$db->query();
		$result = $db->loadAssocList();
	...
	function getStudentsPassByQuizzId(){
		...
		$sql = "SELECT q1.`user_id`, q1.`quiz_id`, q1.`score_quiz`
				FROM `#__guru_quiz_taken` q1
				WHERE q1.`quiz_id`=".intval($id)." and q1.`time_quiz_taken_per_user` = (select min(`time_quiz_taken_per_user`) from `#__guru_quiz_taken` q2 where q2.`user_id`=q1.`user_id`)
				GROUP BY q1.`user_id`, q1.`quiz_id`";
		$db->setQuery($sql);
		$db->query();
		$result = $db->loadAssocList();
	...
	function getStudentDetails(){
		...
		if(intval($userid) > 0){
			$sql = "select distinct(bc.`course_id`), p.`name`, vl.`completed` from #__guru_buy_courses bc left outer join #__guru_viewed_lesson vl on vl.`pid`=bc.`course_id` and vl.`user_id`=bc.`userid`, #__guru_program p where bc.`userid`=".intval($userid)." and bc.`course_id`=p.`id` and p.`author`=".intval($user->id);
			$db->setQuery($sql);
			$db->query();
			...
			if(isset($courses_ids) && count($courses_ids) > 0){
				foreach($courses_ids as $key=>$course_id){
					...
					$sql = "select count(*) from #__guru_quiz_taken where `user_id`=".intval($userid)." and `pid`=".intval($course_id)." group by `quiz_id`";
					$db->setQuery($sql);
					$db->query();
					$taken = $db->loadColumn();
					$taken = @$taken["0"];
					...
					$sql = "SELECT `score_quiz` FROM `#__guru_quiz_taken` WHERE `id` in (select max(`id`) from `#__guru_quiz_taken` where `user_id`=".intval($userid)." and `pid`=".intval($course_id)." group by `quiz_id` order by `id` desc)";
					$db->setQuery($sql);
					$db->query();
					...
					if(isset($all_score_taken) && count($all_score_taken) > 0){
						...
						foreach($all_score_taken as $key_taken=>$value_taken){
							...
							$sum_taken += ($score_quiz["0"] * 100) / $score_quiz["1"];
						}
					...
					$sql = "select `user_id`, `score_quiz` from #__guru_quiz_taken where `quiz_id`=".intval($final_id)." and `user_id`=".intval($userid);
					$db->setQuery($sql);
					$db->query();
					$result = $db->loadAssocList("user_id");
	...
	function getStudentQuizes(){
		...
		if(isset($ids) && count($ids) > 0){
			...
			if(isset($result) && count($result) > 0){
				foreach($result as $key=>$value){
					$sql = "select `score_quiz`, `time_quiz_taken_per_user` from #__guru_quiz_taken where `user_id`=".intval($userid)." and `pid`=".intval($course_id)." and `quiz_id`=".intval($value["id"])." order by `id` desc limit 0,1";
					$db->setQuery($sql);
					$db->query();
					$score_quiz1 = $db->loadAssocList();
					
					$score_quiz = @$score_quiz1["0"]["score_quiz"];
					$result[$key]["timequizuser"] =  @$score_quiz1["0"]["time_quiz_taken_per_user"];
					
					if(isset($score_quiz) && trim($score_quiz) != ""){
						$temp = explode("|", $score_quiz);
						$correct = $temp["0"];
						$total = $temp["1"];
						$wrong = $total - $correct;
						...
						$result[$key]["final_score"] = $percent;
					}
					else{
	...
	function getStudentQuizCompleted(){
		...
		$sql = "SELECT * FROM #__guru_quiz_taken WHERE `user_id`=".intval($user_id)." and `pid`=".intval($pid);
		$db->setQuery($sql);
		$db->query();
		$result = $db->loadObjectList();
	...
	function getPaidDetails(){
		...
		$author_id = $user->id;
		$and = "";
		$filter_course = JRequest::getVar("filter_course", "");
		...
		$limitstart = JRequest::getVar("limitstart", "0");
		$old_limit = JRequest::getVar("old_limit", "0");
	...
	function getPendingDetails(){
		$and = "";
		$filter_promocode = JRequest::getVar("filter_promocode", "");
		$filter_course = JRequest::getVar("filter_course", "");
