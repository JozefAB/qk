...
class guruModelguruOrder extends JModelLegacy {
	...
	function getMyOrders(){
		$db = JFactory::getDBO();
		$search = JRequest::getVar("search", ""); // view only orders for one course
		$course = JRequest::getVar("course", ""); // view only orders for one course
	...
	function getAuthorName($id){ 
		...
		$sql = "SELECT name FROM #__users
				WHERE id = (SELECT author FROM #__guru_program WHERE id=".$id.")";
		$db->setQuery($sql);
		$db->query();
		$result = $db->loadResult();
		return $result;
	...
	function countQuizzTakenF($user_id, $id){
		$db = JFactory::getDBO();
		$sql = "SELECT count(*) from #__guru_quiz_taken WHERE `user_id` =".intval($user_id)." and pid=".$id;
		$db->setQuery($sql);
		$db->query();
		$result = $db->loadResult();	
		return $result;	
	...
	function getScoreQuizF($quiz_id,$user_id,$id){
		...
		$db = JFactory::getDBO();	
		$sql = "SELECT `answers_gived` FROM #__guru_quiz_question_taken qq, #__guru_questions q  WHERE qq.question_id=q.id and user_id=".intval($user_id)." and show_result_quiz_id=".intval($id)." ORDER BY q.reorder";
		$db->setQuery($sql);
		$db->query();
	...
	function getAnsGivedFr($user_id, $id, $quiz_id){
		...
		if($isfinal == 0 && @$show_nb_quiz_select_up == 1){
			$sql = "SELECT qq.`answers_gived` FROM #__guru_quiz_question_taken qq INNER JOIN #__guru_questions q ON( qq.question_id = q.id) WHERE qq.user_id=".intval($user_id)." and qq.show_result_quiz_id=".intval($id)." order by qq.question_order_no";
		}
	...
	function getAnsRightF($quiz_id){
		...
		if($isfinal == 0){
			$sql = "SELECT `answers` FROM #__guru_questions WHERE qid=".intval($quiz_id). " ORDER BY `reorder` ";
		}
		else{
			...
			$sql = "SELECT `answers` FROM #__guru_questions WHERE qid IN (".implode(",", $result_qids).")  ORDER BY `reorder` ";
		}
		$db->setQuery($sql);
		$db->query();
	...
	function getAnsRightFr($quiz_id, $id ){
		...
		if($isfinal == 0 && @$show_nb_quiz_select_up == 1){
			$sql = "SELECT w1.`answers` FROM #__guru_questions w1 INNER JOIN #__guru_quiz_question_taken w2 ON (w1.id= w2.question_id)  WHERE qid=".intval($quiz_id). " and w2.show_result_quiz_id=".intval($id)." order by question_order_no";
		}
		elseif($isfinal == 0 && @$show_nb_quiz_select_up == 0){
			$sql = "SELECT w1.`answers` FROM #__guru_questions w1 INNER JOIN #__guru_quiz_question_taken w2 ON (w1.id= w2.question_id)  WHERE qid=".intval($quiz_id). " and w2.show_result_quiz_id=".intval($id)." order by question_order_no";
		}
		elseif($isfinal == 1 && @$show_nb_quiz_select_up == 0){
			$sql = "SELECT w1.`answers` FROM #__guru_questions w1 INNER JOIN #__guru_quiz_question_taken w2 ON (w1.id= w2.question_id)  WHERE w2.show_result_quiz_id=".intval($id)." order by question_order_no";
		}
		elseif($isfinal == 1 && @$show_nb_quiz_select_up == 1){
			$sql = "SELECT w1.`answers` FROM #__guru_questions w1 INNER JOIN #__guru_quiz_question_taken w2 ON (w1.id= w2.question_id)  WHERE w2.show_result_quiz_id=".intval($id)." order by question_order_no";
		}
		...
		if($isfinal == 0 && @$show_nb_quiz_select_up == 1){
		 $sql = "SELECT question_id FROM #__guru_quiz_question_taken WHERE show_result_quiz_id=".intval($id)." order by question_order_no";
		 $db->setQuery($sql);
		 $db->query();
		 $result=$db->loadObjectList();
		}
	...
	function getAllAnsTextF($quiz_id, $id){
		...
		elseif(isset($result1[0]["nb_quiz_select_up"]) && $result1[0]["nb_quiz_select_up"] !="" && $result1[0]["show_nb_quiz_select_up"] == 0 ){
			$sql = "SELECT question_id FROM #__guru_quiz_question_taken qq, #__guru_questions q  WHERE qq.question_id=q.id and show_result_quiz_id=".$id." ORDER BY qq.id DESC LIMIT 0, ".$result1[0]["nb_quiz_select_up"]."";
			$db->setQuery($sql);
			$db->query();
			$result=$db->loadObjectList();
		}
		else{
			$sql = "SELECT question_id FROM #__guru_quiz_question_taken qq, #__guru_questions q  WHERE qq.question_id=q.id and show_result_quiz_id=".$id." ORDER BY reorder LIMIT 0, ".$result1[0]["nb_quiz_select_up"].""; 
			$db->setQuery($sql);
			$db->query();
			$result=$db->loadObjectList();
		}
	 ...
	 function getQuestionNameF($id,$quiz_id){
		$db = JFactory::getDBO();
		$questions_ids = $_SESSION["questionsids"];
		...
		if($isfinal == 0 && @$show_nb_quiz_select_up ==1){
			$sql = "SELECT `text` FROM #__guru_questions WHERE qid=".intval($quiz_id)." ORDER BY reorder";
		}
		elseif($isfinal == 0 && @$show_nb_quiz_select_up ==0){
			$sql = "SELECT q1.`text`, q2.`question_order_no`  FROM #__guru_questions q1 INNER JOIN #__guru_quiz_question_taken q2 ON (q1.id = q2.question_id)  WHERE q1.qid=".intval($quiz_id)." and q2.show_result_quiz_id=".intval($id)." order by q2.question_order_no";
		}
		else{
			$sql = "SELECT q1.`text` FROM #__guru_questions q1 INNER JOIN #__guru_quiz_question_taken q2 ON (q1.id = q2.question_id)  WHERE  q2.show_result_quiz_id=".intval($id)." order by q2.question_order_no";
		}
