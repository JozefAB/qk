...
class guruModelguruDays extends JModelLegacy {
	...
	function getprogramname () {
			$sql = "SELECT * FROM #__guru_program WHERE id= (SELECT pid FROM #__guru_days WHERE id= ".$_GET['cid'].") ";
			$programname = $this->_getList($sql);
			return $programname;
	}
	
	function getlistTask ($dayid) {
			//$sql = "SELECT * FROM #__guru_task WHERE id in (SELECT media_id FROM #__guru_mediarel WHERE type='dtask' AND type_id = ".$_GET['cid'].") "; 
			
			//$listTask = $this->_getList($sql);
			//return $listTask;
