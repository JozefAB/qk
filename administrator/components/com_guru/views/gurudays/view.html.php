...
class guruAdminViewguruDays extends JViewLegacy {
	...
	function list_all($name, $category_id, $selected_categories=Array(), $size=10, $toplevel=true, $multiple=false) {
		...
		if (isset($_POST['category'])) $category_id=intval($_POST['category']);
		echo "<select onchange=\"document.form1.submit()\" class=\"inputbox\" name=\"$name\">\n";
		if( $toplevel ) {
			$selectedall = '';
			$selectedtop = '';
			if (isset($_POST['category']) && $_POST['category']=='50000') $selectedall=' selected="selected" ';
			if (isset($_POST['category']) && $_POST['category']=='0') $selectedtop=' selected="selected" '; 
