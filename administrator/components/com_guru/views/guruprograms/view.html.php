...
class guruAdminViewguruPrograms extends JViewLegacy {
	...
	function display ($tpl =  null ) { 
		...
		if(isset($_POST['course_publ_status'])) {
			$_SESSION['course_publ_status']=$_POST['course_publ_status'];
		}
		
		if(isset($_POST['course_lock_status'])) {
			$_SESSION['course_lock_status']=$_POST['course_lock_status'];
		}
	...
	*/
	...
	function list_all2($search, $name, $category_id, $selected_categories=Array(), $size=1, $toplevel=true, $multiple=false) {
		...
		$multiple = $multiple ? "multiple=\"multiple\"" : "";
		if (isset($_POST['catid'])) $category_id=intval($_POST['catid']);
		if (isset($_POST['catid']) && intval($_POST['catid']) == 0) $selected= 'selected="selected"';
			else $selected = '';
		$search_submit = '';
		if($search==1)
			$search_submit = " onchange=\"document.adminForm.submit()\" ";		
