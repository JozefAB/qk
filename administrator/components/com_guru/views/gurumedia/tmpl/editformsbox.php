...
<table border="0" width="100%" class="adminform">
		...
		if($_row->type=='video' || (isset($_POST['type']) && $_POST['type'] =='video'))
			{
		...
		if($_row->type=='audio' || (isset($_POST['type']) && $_POST['type'] =='audio'))
			{
		...
		if($_row->type=='docs' || (isset($_POST['type']) && $_POST['type'] =='docs'))
			{
		...
		if($_row->type=='image' || (isset($_POST['type']) && $_POST['type'] =='image'))
			{
				$styleupload = ' ';								
			}			
		?>
		...
		<tr>
			...
			<td>
			 ...
			 <table cellspacing="0" cellpadding="5" border="0" width="100%">
				...
				<td colspan="2" valign="top">
					<table>
						...
						<tr id="to_hide_row" style="display:none">
							...
							<td>
							<?php 
								if(isset($_POST['image'])) $the_file = $_POST['image']; else $the_file = $_row->local;
							?>
							<?php echo $the_file;  /* <img src="<?php echo str_replace('/administrator','/',JURI::base()); echo $configuration->imagesin.'/';?><?php if(isset($_POST['image'])) echo $_POST['image']; else echo $categ->image;?>" alt="" />*/ ?>
							</td>							
						</tr>
					</table>
				</td>
			  ...
			  <?php if($_row->type=='audio' || (isset($_POST['type']) && $_POST['type'] =='audio')) {?>
			  <!-- #eeeeee -->
			  ...
			  <?php } elseif($_row->type=='docs' || (isset($_POST['type']) && $_POST['type'] =='docs')){?>
					...
					<tr>
						...
						<a id="ya4" class="" onclick="javascript:on_click_size('ysize4');" onmouseover="javascript:on_over_size('ysize4');" onmouseout="javascript:on_out_size('ysize4');">
							<div class="">
								...
								?>>
									<div class="yborder"><?php //var_dump($_row->width); ?></div>
								</div>
							</div>
						</a>
		...
		<tr>
			...
			<td>
			<?php 
				if(isset($_POST['image'])) $the_image = $_POST['image']; else $the_image = $_row->local;
			...
			?>
			<?php echo $media_image; /* <img src="<?php echo str_replace('/administrator','/',JURI::base()); echo $configuration->imagesin.'/';?><?php if(isset($_POST['image'])) echo $_POST['image']; else echo $categ->image;?>" alt="" />*/ ?>
			</td>
		</tr>			
		</table>
		</td>
		...
		<tr>
			...
			<td width="80%">
				<?php if (isset($_POST['text'])) $editor_text = stripslashes($_POST['text']); else $editor_text = stripslashes($_row->code);
		...
		<input type="hidden" name="image" value="<?php if(isset($_POST['image'])) echo $_POST['image']; else echo $_row->local;?>" />
