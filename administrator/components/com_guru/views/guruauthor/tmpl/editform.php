	...
	Joomla.submitbutton = function(pressbutton){
		...
		if(username != ""){
			...
			var req = new Request.HTML({
				async: false,
				method: 'get',
				url: '<?php echo JURI::root()."administrator/components/com_guru/js/ajax.php?task=checkExistingUser";?>&username='+username+'&email='+email+'&id=<?php echo $user->id; ?>',
				data: { 'do' : '1' },
...
<?php
	...
	//echo'<pre>'; print_r($config_author); die();
	...
	WHERE author =".intval(JRequest::getVar("id","0","get","int"))." and id IN(SELECT distinct idcourse from #__guru_kunena_courseslinkage) ";
	$db->setQuery($sql);
	$db->query();
	...
	$sql = "select id, commission_plan from #__guru_commissions ORDER BY default_commission DESC";
	$db->setQuery($sql);
	$db->query();
...
<form action="index.php" id="adminForm" method="post" name="adminForm" enctype="multipart/form-data">
     <div class="row-fluid">
								...
								<?php
									if(isset($result_commission) && count($result_commission) > 0){
										foreach($result_commission as $key=>$commission){
											...
											echo '<option value="'.$commission["id"].'" '.$selected.'>'.$commission["commission_plan"].'</option>';
										}
									}
									else{
                                ?>
		...
		<?php		
				$kunenayn = "SELECT forum_kunena_generated FROM #__guru_authors where userid=".intval(JRequest::getVar("id","0","get","int"));
