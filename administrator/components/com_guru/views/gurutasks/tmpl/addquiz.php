...
function addmedia (idu, name, asoc_file, description) {
	...
	<div style="float: left; font-weight:bold"><?php echo JText::_("GURU_CLICK_TO_QUIZ"); ?></div>
		<br /><br />
	<div>
<form name="adminForm2" action="index.php?option=com_guru&controller=guruTasks&task=addquiz&med=<?php echo $_GET['med'];?>&tmpl=component&cid[]=<?php echo $_GET['cid'][0];?><?php if(isset($_GET['quiz'])){echo "&quiz=".$_GET['quiz'];}?><?php if(isset($_REQUEST['type'])){echo "&type=".$_REQUEST['type'];}?>" method="post">
...
<?php 
	...
	<form name="adminForm" id="adminForm" action="index.php?option=com_guru&controller=guruTasks&task=addquiz&med=<?php echo $_GET['med'];?>&tmpl=component&cid[]=<?php echo $_GET['cid'][0];?>" method="post">
		<table>
        	<tr>
        		<td colspan="6">
