...
<script type="text/javascript" language="javascript">
	function editquestion (qid, id) {
		...
		if (document.adminForm.text.value=='') {
			...
			window.parent.document.getElementById('tdq'+qid).innerHTML = '<a style="color:#666666!important; text-decoration:underline !important;" class="modal" rel="{handler: \'iframe\', size: {x: 770, y: 400}}" href="index.php?option=com_guru&controller=guruQuiz&task=editquestion&no_html=1&cid[]='+<?php echo $_GET['cid'][0]; ?>+'&qid='+qid+'">'+document.getElementById('text').value+'</a>';
...
<form method="post" name="adminForm" id="adminForm" action="index.php">
     ...
     <table id="g_quize_q" width="100%">
		...
		<tr>
			<td valign="top" align="left" style="border-bottom:1px solid #cccccc;">
				...
				<input type="button" class="btn" value="<?php echo JText::_("GURU_SAVE_PROGRAM_BTN"); ?>" onclick="editquestion(<?php echo $_GET['qid'].','.$_GET['cid'][0];?>)">
			</td>
    ...
    <input type="hidden" value="<?php echo intval($_GET['qid']);?>" name="qid"/>
