...
class guruAdminModelguruLanguage extends JModelLegacy {
	...
	function store () {
		...
		// we update the ADMIN LANGUAGE FILE - begin
		$admin= JPATH_SITE . "/administrator/language/en-GB/en-GB.com_guru.ini" ;
		@chmod($admin,0777);
		$fp=@fopen($admin,"w");
		$the_admin_file = stripslashes(JArrayHelper::getValue($_POST,'admin_file'));
		
		fwrite($fp,$the_admin_file);
		fclose($fp);
		// we update the ADMIN LANGUAGE FILE - end
		
		// we update the FRONT END LANGUAGE FILE - begin
		$front=JPATH_SITE . "/language/en-GB/en-GB.com_guru.ini" ;
		@chmod($front,0777);
		$fp=@fopen($front,"w");
		$the_front_file = str_replace("\\", '', stripslashes(JArrayHelper::getValue($_POST,'front_file')));
		fwrite($fp,$the_front_file);
		fclose($fp);
