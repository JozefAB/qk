...
class guruAdminModelguruMedia extends JModelLegacy {
	...
	function getFilters(){
		...
		$db = JFactory::getDBO();
		@$filter->search_media	= $db->getEscaped($app->getUserStateFromRequest("search_media","search_media","","string"));
...
<embed type="application/x-mplayer2" pluginspage="http://www.microsoft.com/Windows/MediaPlayer/" src="'.JURI::root().$configs->videoin."/".$media->local.'" name="MediaPlayer1" width="'.$media->width.'" height="'.$media->height.'" autostart="1" showcontrols="1" volume="10">
	...
	if($the_media->type=='quiz')
		{
			...
			$q  = "SELECT * FROM #__guru_questions WHERE qid = ".$the_media->source;
			$db->setQuery( $q );
	...
	function saveMass(){
		$cid = JRequest::getVar("cid", array(), "post", "array");
		$image = JRequest::getVar("image", array(), "post", "array");
		$title = JRequest::getVar("title", array(), "post", "array");
		$url = JRequest::getVar("url", array(), "post", "array");
		$description = JRequest::getVar("description", array(), "post", "array");
		$category_id = JRequest::getVar("category_id", "0");
		$teacher_id = JRequest::getVar("teacher_id", "0");
		$step_access = JRequest::getVar("step_access", "2");
