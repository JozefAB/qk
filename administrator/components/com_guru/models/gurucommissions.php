...
class guruAdminModelguruCommissions extends JModelLegacy{
	...
	function getPendingCommissionsToBePaid(){
		...
		$filter_payment = JRequest::getVar("filter_payment", "0");
		$opt = "";
		$and = "";
	...
	function make_paid_top(){
		$db = JFactory::getDBO();
		$cids = JRequest::getVar("cid", array(), "post", "array");
		$cids_boottom = JRequest::getVar("course_second_table", array(), "post", "array");
		$currency = JRequest::getVar("currency_row","");
		if(count($cids)>0){
			$cids = $cids;
			$cid = explode("-",$cids["0"]);
		}
	...
	function getPendingDetails($course_id, $author_id, $pageto){
		$search = JRequest::getVar("search_text", "");
		$filter_commission_plan = JRequest::getVar("filter_commission_plan", "");
		$filter_paymentmethod = JRequest::getVar("filter_paymentmethod", "");
		$filter_promocode = JRequest::getVar("filter_promocode", "");
		$id = JRequest::getVar("id", "0");
		$block = JRequest::getVar("block", "0");
		$pagefrom = JRequest::getVar("page","");
