...
class NewsRss
{
	...
	{
		$helper = new NewsRss_Misc();
		$this->rss_url = $helper->fix_protocol($url, 1);
	}
	...
	{	$helper = new NewsRss_Misc();
		if ($helper->is_a_class($class, 'NewsRss_Cache'))
		{
	...
	{
		$helper = new NewsRss_Misc();
		if ($helper->is_a_class($class, 'NewsRss_Locator'))
		{
	...
	{	$helper = new NewsRss_Misc();
		if ($helper->is_a_class($class, 'NewsRss_Parser'))
		{
	...
	{	$helper = new NewsRss_Misc();
		if ($helper->is_a_class($class, 'NewsRss_File'))
		{
	...
	function init()
	{
		...
		if ($this->sanitize->bypass_image_hotlink && !empty($_GET[$this->sanitize->bypass_image_hotlink]))
		{
			...
			$helper->display_file($_GET[$this->sanitize->bypass_image_hotlink], 10, $this->useragent);
		}
...
EOT;
		...
		if (!empty($this->rss_url) || !empty($this->raw_data))
		{
				...
				if (!$locate->is_feed($file))
				{
					...
					if ($feed)
					{
						if ($cache && !$cache->save(array('url' => $this->rss_url, 'feed_url' => $feed)))
						{
							...
							$helper->error($this->error, E_USER_WARNING, __FILE__, __LINE__);
						}
						$this->rss_url = $feed;
						return $this->init();
					}
			...
			$data = $helper->change_encoding($data, $encoding, 'UTF-8');
			...
			if (version_compare(phpversion(), '5', '<'))
			{
				...
				{
					$data = $helper->utf8_bad_replace($data);
				}
			}
	...
	{	$helper = new NewsRss_Misc();
		...
		{
			$favicon = $helper->absolutize_url('/favicon.ico', $this->get_feed_link());
	...
	{	$helper = new NewsRss_Misc();
		if (!empty($this->rss_url))
		{
			return $helper->fix_protocol($this->rss_url, 2);
		}
		else
		{
			return false;
	...
	function subscribe_outlook()
	{
		...
		if (!empty($this->rss_url))
		{
			return 'outlook' . $helper->fix_protocol($this->rss_url, 2);
		}
		else
		{
			return false;
	...
	{	$helper = new NewsRss_Misc();
		if (!empty($this->rss_url))
		{
			return $helper->fix_protocol($this->rss_url, 3);
		}
		else
		{
			return false;
...
class NewsRss_File
{
	...
	{ $helper = new NewsRss_Misc();
		...
		if (preg_match('/^http(s)?:\/\//i', $url))
		{
			...
			if (extension_loaded('curl') && version_compare($helper->get_curl_version(), '7.10.5', '>=') && !$force_fsockopen)
			{
				$this->method = 'curl';
				$fp = curl_init();
				$headers2 = array();
...
class NewsRss_Misc
{
	function absolutize_url($relative, $base)
	{
		...
		if (!empty($relative))
		{
			$relative = $helper->parse_url($relative, false);
			...
			{
				$base = $helper->parse_url($base, false);
				...
				else
				{
					...
					if (!empty($relative['path']))
					{
						...
						while (!empty($input))
						{
							...
							else if (strpos($input, '/.') === 0 && $helper->strendpos($input, '/.') === 0)
							{
	...
	function fix_protocol($url, $http = 1)
	{
		...
		{
			return $helper->fix_protocol("$parsed[authority]$parsed[path]$parsed[query]$parsed[fragment]", $http);
		}
		if (!file_exists($url) && empty($parsed['scheme']))
		{
			return $helper->fix_protocol("http://$url", $http);
		}
	...
	{	$helper = new NewsRss_Misc();
		$input = $helper->encoding($input);
		$output = $helper->encoding($output);
...
class NewsRss_Locator
{
	...
	{
		$helper = new NewsRss_Misc();
		$links = $helper->get_element('link', $file->body());
		$done = array();
		foreach ($links as $link)
		{
			if (!empty($link['attribs']['TYPE']['data']) && !empty($link['attribs']['HREF']['data']) && !empty($link['attribs']['REL']['data']))
			{
				...
				$href = $helper->absolutize_url(trim($link['attribs']['HREF']['data']), $this->file->url);
	...
	{	$helper = new NewsRss_Misc();
		$links = $helper->get_element('a', $file->body());
		foreach ($links as $link)
		{
			if (!empty($link['attribs']['HREF']['data']))
			{
				$href = trim($link['attribs']['HREF']['data']);
				$parsed = $helper->parse_url($href);
				if (empty($parsed['scheme']) || $parsed['scheme'] != 'javascript')
				{
					$current = $helper->parse_url($this->file->url);
					if (empty($parsed['authority']) || $parsed['authority'] == $current['authority'])
					{
						$this->local[] = $helper->absolutize_url($href, $this->file->url);
					}
					else
					{
						$this->elsewhere[] = $helper->absolutize_url($href, $this->file->url);
					}
				}
			}
		}
	...
	function extension(&$array)
	{
		...
		{
			$value = $helper->absolutize_url($value, $this->file->url);
...
class NewsRss_Sanitize
{
	...
	{
		$helper = new NewsRss_Misc();
		if ($helper->is_a_class($class, 'NewsRss_Item'))
		{
	...
	function set_author_class($class = 'NewsRss_Author')
	{	$helper = new NewsRss_Misc();
		if ($helper->is_a_class($class, 'NewsRss_Author'))
		{
	...
	{
		$helper = new NewsRss_Misc();
		if ($helper->is_a_class($class, 'NewsRss_Enclosure'))
		{
	...
	{	$helper = new NewsRss_Misc();	
		...
		{
			$this->feed_xmlbase = $helper->parse_url($url);
		...
		if (!empty($data['items']))
		{
			foreach ($data['items'] as $key => $item)
			{
				...
				{
					$this->item_xmlbase = $helper->absolutize_url($item['attribs']['XML:BASE'], $this->feed_xmlbase);
				}
				else if (!empty($item['attribs']['HTTP://WWW.W3.ORG/XML/1998/NAMESPACE:BASE']))
				{
					$this->item_xmlbase = $helper->absolutize_url($item['attribs']['HTTP://WWW.W3.ORG/XML/1998/NAMESPACE:BASE'], $this->feed_xmlbase);
				}
				else
				{
	...
	function sanitize($data, $attribs, $is_url = false, $force_decode = false)
	{
		...
		{
			$images = $helper->get_element('img', $data);
			foreach ($images as $img)
			{
				if (!empty($img['attribs']['SRC']['data']))
				{
					...
					$data = str_replace($img['full'], $helper->element_implode($img), $data);
				}
			}
		}
		...
		$data = $helper->change_encoding($data, $this->input_encoding, $this->output_encoding);
	...
	function replace_urls($data, $raw_url = false)
	{
		...
		if (!empty($xmlbase))
		{
			...
			{
				$xmlbase = $helper->absolutize_url($xmlbase, $this->item_xmlbase);
			}
			else
			{
				$xmlbase = $helper->absolutize_url($xmlbase, $this->feed_xmlbase);
			}
		}
		...
		if ($raw_url)
		{
			return $helper->absolutize_url($data, $xmlbase);
		}
		else
		{
			$attributes = array(
			...
			foreach ($attributes as $attribute)
			{
				...
				{
					$new_tag = str_replace($attrib[1], $helper->absolutize_url($attrib[1], $xmlbase), $attrib[0]);
	...
	function do_entites_decode($data)
	{
		...
		{
			$return = $helper->change_encoding(html_entity_decode($data[0], ENT_QUOTES), 'ISO-8859-1', $this->input_encoding);
			if ($return == $data[0])
			{
				$return = $helper->change_encoding(preg_replace_callback('/&#([x]?[0-9a-f]+);/mi', array(&$this, 'replace_num_entity'), $data[0]), 'UTF-8', $this->input_encoding);
			}
