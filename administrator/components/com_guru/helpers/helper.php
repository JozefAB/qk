...
class guruAdminHelper {
	...
	function create_thumbnails($images, $width, $height, $width_old, $height_old, $type){
		...
		$width_2 = $width;
		$height_2 = $height;
		if(!is_dir(JPATH_ROOT.DS."images".DS."stories".DS."guru_thumb")){
			mkdir(JPATH_ROOT.DS."images".DS."stories".DS."guru_thumb", 0777);
		}	
		if($images == "" ){
			return;
		}	
		...
		if(file_exists(JPATH_ROOT.DS."images".DS."stories".DS."guru_thumb".DS.$type."thumb_".$photo_name)){
			$img_size_thumb = @getimagesize(JPATH_ROOT.DS."images".DS."stories".DS."guru_thumb".DS.$type."thumb_".$photo_name);
		...
		if($ext == "jpg") $upload_th = imagejpeg($image_p, JPATH_ROOT.DS."images".DS."stories".DS."guru_thumb".DS.$name, 100);
		if($ext == "gif") $upload_th = imagegif($image_p, JPATH_ROOT.DS."images".DS."stories".DS."guru_thumb".DS.$name, 100);
		if($ext == "png") $upload_th = imagepng($image_p, JPATH_ROOT.DS."images".DS."stories".DS."guru_thumb".DS.$name, 9);
		@chmod(JPATH_ROOT.DS."images".DS."stories".DS."guru_thumb".DS.$name, 0755);			 
		if($upload_th){
	...
	function jwAllVideos( &$row, $parawidth=300, $paraheight=20, $parvwidth=400, $parvheight=300,$layout_id="",$auto_play) {
		...
		//include(JPATH_SITE.DS."plugins".DS."content".DS."jw_allvideos".DS."includes".DS."helper.php");
