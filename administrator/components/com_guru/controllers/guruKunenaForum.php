	...
	function kunenaoninstall(){
		...
		for($i=0; $i < count($coursenames); $i++){	
			...
			if($countcoursesb == 0){
				...
				if($result1 == 0){
					$sql = "INSERT INTO #__kunena_categories ( `parent_id`, `name`, `alias`, `icon_id`, `locked`, `accesstype`, `access`, `pub_access`, `pub_recurse`, `admin_access`, `admin_recurse`, `ordering`, `published`, `channels`, `checked_out`, `checked_out_time`, `review`, `allow_anonymous`, `post_anonymous`, `hits`, `description`, `headerdesc`, `class_sfx`, `allow_polls`, `topic_ordering`, `numTopics`, `numPosts`, `last_topic_id`, `last_post_id`, `last_post_time`, `params`) VALUES ( '".$idmainforum."', '".$coursenames[$i]["name"]."', '".$coursenames[$i]['alias']."', 0, 0, 'joomla.group', 1, 1, 1, 8, 1, 2, 1, NULL, 0, '0000-00-00 00:00:00', 0, 0, 0, 0, '', '', '', 0, 'lastpost', 0, 0, 0, 0, 0, '{}')";
					$db->setQuery($sql);
					$db->query($sql);
				}
				...
				for($k = 0; $k <count($aliasmodules); $k++ ){
						...
						if(count($resultt) == 0){
							$sql = "INSERT INTO #__kunena_categories ( `parent_id`, `name`, `alias`, `icon_id`, `locked`, `accesstype`, `access`, `pub_access`, `pub_recurse`, `admin_access`, `admin_recurse`, `ordering`, `published`, `channels`, `checked_out`, `checked_out_time`, `review`, `allow_anonymous`, `post_anonymous`, `hits`, `description`, `headerdesc`, `class_sfx`, `allow_polls`, `topic_ordering`, `numTopics`, `numPosts`, `last_topic_id`, `last_post_id`, `last_post_time`, `params`) VALUES ( ".$resultid.", '".$aliasmodules[$k]['title']."', '".$aliasmodules[$k]['alias']."', 0, 0, 'joomla.group', 1, 1, 1, 8, 1, '".($k+1)."', 1, NULL, 0, '0000-00-00 00:00:00', 0, 0, 0, 0, '', '', '', 0, 'lastpost', 0, 0, 0, 0, 0, '{}')";
							$db->setQuery($sql);
							$db->query($sql);
					...
					for($j = 0; $j<count($aliaslesson); $j++){
						...
						if(count($result2) == 0){
							$sql = "INSERT INTO #__kunena_categories ( `parent_id`, `name`, `alias`, `icon_id`, `locked`, `accesstype`, `access`, `pub_access`, `pub_recurse`, `admin_access`, `admin_recurse`, `ordering`, `published`, `channels`, `checked_out`, `checked_out_time`, `review`, `allow_anonymous`, `post_anonymous`, `hits`, `description`, `headerdesc`, `class_sfx`, `allow_polls`, `topic_ordering`, `numTopics`, `numPosts`, `last_topic_id`, `last_post_id`, `last_post_time`, `params`) VALUES ( ".$resultidmodule.", '".$aliaslesson[$j]['name']."', '".$aliaslesson[$j]['alias']."', 0, 0, 'joomla.group', 1, 1, 1, 8, 1, '".($j+1)."', 1, NULL, 0, '0000-00-00 00:00:00', 0, 0, 0, 0, '', '', '', 0, 'lastpost', 0, 0, 0, 0, 0, '{}')";
							$db->setQuery($sql);
							$db->query($sql);
						}
					 ...
					 $sql = "INSERT INTO #__guru_kunena_lessonslinkage (`idlesson`, `lessonname`, `catidkunena`) VALUES (  '".$aliaslesson[$j]['id']."', '".$aliaslesson[$j]['name']."', '".$resultidlesson."')";
					 $db->setQuery($sql);
					 $db->query($sql);
					 ...
					 $sql = "UPDATE #__kunena_categories set name='".$aliaslesson[$j]['name']."' WHERE id=".intval($catidkunena);
					 $db->setQuery($sql);
					 $db->query();
				}
			}
