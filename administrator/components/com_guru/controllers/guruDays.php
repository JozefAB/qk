...
class guruAdminControllerguruDays extends guruAdminController {
	...
	/*function apply(){
		...
		$node	= JRequest::getVar("node","1","post","int");
		$pid    = JRequest::getVar("pid","0","post","int");
		$result = $this->_model->store();
		if($result==false){
			$msg = JText::_('GURU_DAY_APPLY');
		}
		else{
			$msg = JText::_('GURU_DAY_APPLY');
		}
		$link = "index.php?option=com_guru&controller=guruDays&tmpl=component&task=edit&node=".$node."&cid[]=".$id;		
		$this->setRedirect($link, $msg);
	}	*/
	...
	function make_duplicate(){
		$the_days=JRequest::getVar("the_days","","post","string");
		$the_days = substr($the_days, 0, strlen($the_days)-1);
		$the_days = explode(',', $the_days);
		$the_program_id = JRequest::getVar('program_id',"0","post","int");
