...
class RSMembershipHelper
{
	...
	public static function showCustomField($field, $selected=array(), $editable=true, $show_required=true, $type='user')
	{
		...
		$return = array();
		$return[0] = '<label title="'.JText::_($field->label).'" class="fs-field-label fs-anim-upper hasTip" for="rsm_'.$field->name.'" id="jform_'.$field->name.'-lbl">'.JText::_($field->label).'</label>';
		$return[1] = '';
		switch ($field->type)
		{
			case 'hidden':
			...
			case 'textbox':
				...
				$return[1] = '<input type="text" name="'.$name.'" class="fs-anim-lower" id="rsm_'.$field->name.'" value="'.htmlspecialchars($field->values, ENT_COMPAT, 'utf-8').'" size="40" required />';
